...
const LoginPage = () => {
    ...
    const login = useMutation(newLogin => {
      return instance.post('/users/login', newLogin)
    }) 
    ...
    const handleSubmit = async (e) => {
      ...
      if(res.data) {
        ...
        const me = instance.get('/users/me')
        await setUser((await me).data);
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12">
        <div className="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
          <h1 className="font-bold text-center text-2xl mb-5">Войти</h1> 
          <div className="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
            <form onSubmit={handleSubmit} className="px-5 py-7">
              ...
              <button type="submit" className="transition duration-200 bg-blue-500 hover:bg-blue-600 focus:bg-blue-700 focus:shadow-sm focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block">
                  <button type="submit" className="inline-block mr-2">Войти</button>
              ...
              <div className="grid grid-cols-2 gap-1">
                <div className="text-center sm:text-left whitespace-nowrap">
                  <button className="transition duration-200 mx-5 px-5 py-4 font-normal text-sm rounded-lg text-gray-500">
                      <span className="inline-block ml-1">Нет аккаунта?</span>
                  </button>
                </div>
                <div className="text-center sm:text-right whitespace-nowrap">
                  <button className="transition duration-200 mx-5 px-5 py-4 cursor-pointer font-normal text-sm rounded-lg text-gray-500 hover:bg-gray-100 focus:outline-none focus:bg-gray-200 focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50 ring-inset">
                      ...
                      <span onClick={() => navigate('/auth/register')} className="inline-block ml-1">Регистрация</span>
                  </button>
                </div>
              </div>
            </div>
